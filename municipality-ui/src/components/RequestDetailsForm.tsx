...
export default function RequestDetailsForm(props: RequestDetailsFormProps) {
  const { onBack, onNext, requireAdditionalDetails = false, initialAdditionalDetails } = props;
  const { initialFields } = props;
  const [additionalDetails, setAdditionalDetails] = useState<string>(initialAdditionalDetails || '');
  const [error, setError] = useState<string>('');

  useEffect(() => {
    if (initialAdditionalDetails !== undefined) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setError('');
    onNext({ additionalDetails: additionalDetails || '', fields: initialFields || {} });
  };

  return (
    <div dir="rtl" lang="he" style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '0 12px 20px', width: '100%', boxSizing: 'border-box', margin: 0 }}>
